...
glm::mat4 KeyframeCollection::interpolateTransformation( float time ) const
{
    ...
    if( !m_keyframes.empty() )
    {
        ...
        glm::quat orientation = glm::quat(glm::slerp(result[0].second.getOrientation(),result[1].second.getOrientation(),factor));
        glm::vec3 translation = glm::vec3(glm::lerp(result[0].second.getTranslation(),result[1].second.getTranslation(),factor));
        glm::vec3 scale = glm::vec3(glm::lerp(result[0].second.getScale(),result[1].second.getScale(),factor));
...
glm::mat4 KeyframeCollection::parametricInterpolationTransformation( float time, int order ) const
{
    //TODO: Complete the interpolation framework
    if( !m_keyframes.empty() )
    {
